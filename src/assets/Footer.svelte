<script>
  const currentYear = new Date().getFullYear();
</script>

<footer class="bg-[#1b1b1b] text-white py-16 relative overflow-hidden font-khula" id="footer-section">
  <!-- Background gradient effect -->
  <div class="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent pointer-events-none"></div>
  
  <!-- Footer content -->
  <div class="container mx-auto px-6 md:px-12 relative z-10">
    <!-- Logo and tagline -->
    <div class="flex flex-col md:flex-row justify-between mb-12 md:mb-16">
      <div class="mb-10 md:mb-0">
        <div class="flex items-center mb-4">
          <span class="text-2xl font-semibold font-alconica">vexos</span>
        </div>
        <p class="text-white/70 max-w-md">
          Transforming location intelligence for business growth through AI and geospatial analytics
        </p>
      </div>

      <!-- Contact & Social links -->
      <div class="flex flex-col md:items-end">
        <p class="text-white/70 mb-3">Get in touch</p>

        <!-- Social media icons -->
        <div class="flex space-x-4">
          <a href="https://www.linkedin.com/company/vexos/" target="_blank" rel="noopener noreferrer"
             class="text-white/70 hover:text-[#0077b5] transition-colors"
             aria-label="Visit Vexos LinkedIn page">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
              <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
            </svg>
          </a>
          <a href="https://twitter.com/vexos_ai" target="_blank" rel="noopener noreferrer"
             class="text-white/70 hover:text-[#1DA1F2] transition-colors"
             aria-label="Visit Vexos Twitter page">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
              <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/>
            </svg>
          </a>
        </div>
      </div>
    </div>

    <!-- Bottom section with links and copyright -->
    <div class="border-t border-white/10 pt-8 flex flex-col md:flex-row md:items-center justify-between">
      <nav class="mb-6 md:mb-0">
        <ul class="flex flex-wrap gap-6">
          <li><a href="/" class="text-white/70 hover:text-white transition-colors">Home</a></li>
          <li><a href="#what-we-do" class="text-white/70 hover:text-white transition-colors">What We Do</a></li>
          <li><a href="#why-vexos" class="text-white/70 hover:text-white transition-colors">Why Vexos</a></li>
          <li><a href="#team-section" class="text-white/70 hover:text-white transition-colors">Team</a></li>
        </ul>
      </nav>

      <p class="text-white/50 text-sm">
        © {currentYear} Vexos. All rights reserved.
      </p>
    </div>
  </div>

</footer>
